...
def chat():
    ...
    def generate():
        ...
        with client.messages.stream(
            ...
            # 생성된 문자열 조각을 받을 때 마다 SSE 형식으로 데이터를 보내기
            # 클라이언트는 "data: JSON 문자열" 형식으로 응답을 받는다.

            # 아래의 Response에 의해 generate 가 반환하는 제네레이터 객체의 next 메서드를 호출하며 이 코드를 동작시킨다.
            # stream.text_stream 은 동기이기 때문에 Claude가 텍스트를 보내오기 전까지 잠시 멈췄다가
            # 텍스트가 도착하면 클라이언트로 전송하고 next 메서드가 호출되어 다음 텍스트를 기다리는 과정을 반복한다.
    ...
    # 인수로 제네레이터 객체를 전달함으로써 제네레이터를 반복하며 yield하는 텍스트가 담긴 문자열을 보낸다.
